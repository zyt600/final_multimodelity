...
from loader.youcook_loader import Youcook_DataLoader
...
def main(args):
    ...
    test_dataset = Youcook_DataLoader(data='./data/validation_youcook.csv', num_clip=args.num_windows_test,
                                video_root=args.eval_video_root, fps=args.fps, num_frames=args.num_frames, size=args.video_size, crop_only=False,
                                center_crop=True,)
...
def main_worker(gpu, ngpus_per_node, main, args):
    ...
    ip = s.getsockname()[0]
    args.dist_url = f'tcp://localhost:5555'
